...
void loadelf(char *filename, struct PCB *p1)
{
    ...
    for(i=0;i<elf->e_phnum;i++)
    {
        if(phdr->p_type==1) {
            ...
            for(addr=phdr->p_vaddr;addr<phdr->p_vaddr+phdr->p_memsz;addr+=4096)
            {
                ...
                map_user_address(addr,phy,4096,(struct pml4t *)((uint64_t)p1->page_table+KERNBASE));
    ...
    map_user_address((uint64_t)stack,(uint64_t)stack-USERBASE,4096,(struct pml4t *)((uint64_t)p1->page_table+KERNBASE));
    memset((void *)stack,0,4096);
    kprintf("\nValue of stack: %x", (uint64_t)stack);
    vma = bump(sizeof(struct vm_area_struct));
    vma->vma_file = NULL;
    vma->vma_start = (uint64_t)stack;
    vma->vma_end = (uint64_t)stack + 4096;
    ...
    tempvma->next = vma;
    vma->next = NULL;
    p1->rsp = (uint64_t)stack+4095;
    *(uint64_t *)((uint64_t)stack+4095)=(uint64_t)on_completion_pointer;
